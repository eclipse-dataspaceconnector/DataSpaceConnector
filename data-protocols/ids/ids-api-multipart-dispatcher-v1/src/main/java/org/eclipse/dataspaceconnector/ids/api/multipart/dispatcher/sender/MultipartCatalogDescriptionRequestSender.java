        ...
        if (catalog.getProperties() != null) {
            for (Map.Entry<String, Object> entry : catalog.getProperties().entrySet()) {
                if ("ids:offeredResource".equals(entry.getKey())) {
                    JsonNode node = getObjectMapper().convertValue(entry.getValue(), JsonNode.class);
                    ...
                    for (JsonNode objNode : node.get("objectList")) {
                        ...
                        resource.putAll(getObjectMapper().convertValue(objNode, Map.class));
                        offeredResources.add(getObjectMapper().convertValue(resource, Resource.class));
                    }
